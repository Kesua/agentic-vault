...
$pluginRoot = Join-Path $env:USERPROFILE "plugins\playwright-browser"
$serverPath = Join-Path $pluginRoot "scripts\server.mjs"
$skillPath = Join-Path $pluginRoot "skills\playwright-browser-use\SKILL.md"
...
New-Item -ItemType Directory -Force -Path (Split-Path $serverPath) | Out-Null
New-Item -ItemType Directory -Force -Path (Split-Path $pluginJsonPath) | Out-Null
...
'@
Write-Utf8NoBom -LiteralPath $pluginJsonPath -Value $pluginJson
...
'@
Write-Utf8NoBom -LiteralPath $packageJsonPath -Value $packageJson
...
Write-Utf8NoBom -LiteralPath $mcpPath -Value $mcpJson
...
'@
Write-Utf8NoBom -LiteralPath $skillPath -Value $skillContent
...
'@
Write-Utf8NoBom -LiteralPath $serverPath -Value $serverSource
...
$marketplace.name = "jan-local-plugins"
if (-not $marketplace.interface) {
  $marketplace | Add-Member -NotePropertyName interface -NotePropertyValue ([pscustomobject]@{})
}
$marketplace.interface.displayName = "Jan Local Plugins"
$marketplaceText = $marketplace | ConvertTo-Json -Depth 8
Write-Utf8NoBom -LiteralPath $marketplacePath -Value $marketplaceText
