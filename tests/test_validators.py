...
from src.wizard.validators import (
    format_check_todoist,
    live_check_todoist,
    format_check_telegram,
    live_check_telegram,
    format_check_fireflies,
    live_check_fireflies,
    format_check_clockify,
    live_check_clockify,
    format_check_slack,
    live_check_slack,
    format_check_google_client,
    detect_telegram_ids,
)


def test_format_check_todoist():
    assert format_check_todoist("short") == {
        "valid": False,
        "message": "Token is too short",
    }
    assert format_check_todoist("a" * 40) == {
        "valid": True,
        "message": "Format OK",
        "token": "a" * 40,
    }
...
def test_format_check_telegram():
    assert format_check_telegram("invalid") == {
        "valid": False,
    ...
    assert format_check_telegram(valid_token) == {
        "valid": True,
        "message": "Format OK",
        "token": valid_token,
    }
